...
Jarvis Calendar requests use a typed action contract generated by the configured
`deep_120b` planner. Jarvis Core validates that contract, supplies only recent
verified event artifacts for follow-up references, and sends it to the Google
worker for deterministic execution and verification. Personal create, list,
reschedule, and explicit delete actions can run directly; attendee invitations
still require approval. The legacy text parser is retained temporarily and is
identified as `legacy_parser_fallback` in the action trace when used for a
